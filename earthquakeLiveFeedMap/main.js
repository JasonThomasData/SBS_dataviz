...
datavizState.geoJsonData = $.get(datavizState.dataLocation, function(data) { 
	console.log(data)
	return data;
});
datavizState.map = makeNewMap(datavizState);
datavizState.featureLayer = loadFeatures(datavizState);
...
function loadFeatures(datavizState){
	...
	$('#mapDetails').text('Map data generated at ' + moment(data.metadata.generated).format("hh:mm:ss A") + ' on ' + moment(data.metadata.generated).format("DD/MM/YYYY") + ' AEST');
}
